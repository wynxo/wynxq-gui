...
class TerminalLines(QAbstractListModel):
    ...
    def sync(self, rows: list[dict]) -> None:
        ...
        if first_change >= 0:
            ...
            self.dataChanged.emit(self.index(first_change), self.index(end - 1), [self.HTML, self.TEXT])
        if len(rendered) > len(self._rows):
...
class DockController(QObject):
    ...
    def tabs(self): return [{"id": name, **TAB_META[name]} for name in TABS]

    @Property(int, constant=True)
    def minimumWidth(self): return MIN_WIDTH

    @Property(int, constant=True)
    def maximumWidth(self): return MAX_WIDTH
    ...
    def setVisible(self, value):
        ...
        if value: self._on_tab_shown(self._tab)
        else: self._poll.stop()
        self.changed.emit()

    @Slot()
    def toggle(self): self.setVisible(not self._visible)
    ...
    def _select_tab(self, name: str) -> None:
        if name == self._tab:
            self._on_tab_shown(name); return
        self._tab = name
    ...
    def suggest(self, name: str, *, open_dock: bool = False) -> None:
        if name not in TABS or self._tab_pinned: return
        if open_dock and not self._visible: self.setVisible(True)
        if self._visible or open_dock: self._select_tab(name)

    def _on_tab_shown(self, name: str) -> None:
        if not self._visible: return
        if name == "terminal":
            self.startTerminal(); self._poll.start()
        else:
            self._poll.stop()
        if name == "changes": self.refreshChanges()
        if name == "files" and self.tree.root and self.tree.rowCount() == 0: self.tree.reload()

    @Property(str, notify=changed)
    def projectPath(self): return self._project

    @Property(str, notify=changed)
    def projectName(self): return Path(self._project).name if self._project else ""

    def _block_dirty_transition(self, action: str) -> bool:
        if not self._viewer_dirty: return False
    ...
    def set_project(self, path: str) -> bool:
        path = str(path or "")
        if path == self._project: return True
        if self._block_dirty_transition("switching projects"): return False
        ...
        self._changes = {"repository": False, "branch": "", "files": [], "added": 0, "removed": 0, "error": ""}
        if self._shell is not None: self.restartTerminal()
        self.changed.emit(); self.filesChanged.emit(); self.viewerChanged.emit(); self.changesChanged.emit(); self.contextChanged.emit()
        if path: self.refreshChanges()
        return True

    @Property(QObject, constant=True)
    def fileModel(self): return self.tree

    @Property(bool, notify=filesChanged)
    def showHidden(self): return self.tree.show_hidden

    @Property(str, notify=filesChanged)
    def fileFilter(self): return self._file_filter

    @Property("QVariantList", notify=filesChanged)
    def fileMatches(self): return self._search_results

    @Property(str, notify=filesChanged)
    def fileError(self): return self.tree.error
    ...
    def setFileFilter(self, text):
        text = str(text or "")
        if text == self._file_filter: return
    ...
    def _start_file_search(self):
        ...
        def search():
            try:
                return files.search_tree(project, needle, limit=120, show_hidden=show_hidden)
            except (OSError, ValueError):
                return []
    ...
    @Slot(str)
    def toggleFolder(self, path): self.tree.toggle(str(path))

    @Slot(str)
    def revealFile(self, path):
        row = self.tree.reveal(str(path))
        if row >= 0:
            self.tree.set_selected(str(path)); self.revealRow.emit(row)

    @Property("QVariantMap", notify=viewerChanged)
    def file(self): return dict(self._viewer)

    @Property(str, notify=viewerChanged)
    def filePath(self): return str(self._viewer.get("path", ""))

    @Property(bool, notify=viewerChanged)
    def fileModified(self): return self._viewer_dirty

    @Slot(str, result=bool)
    def openFile(self, path):
        if not self._project or not path: return False
        try: target = str(files.resolve_within(self._project, str(path)))
        except (OSError, ValueError): target = str(path)
        if self._viewer_dirty:
            if target == str(self._viewer.get("path", "")): return True
            if self._block_dirty_transition("opening another file"): return False
        try:
            record = files.read_file(self._project, str(path))
        except (OSError, ValueError) as exc:
            name = Path(str(path)).name
            self._viewer = {"path": str(path), "name": name, "lines": 0, "text": "", "error": files.explain(exc, f"“{name}”")}
            self._viewer_buffer = ""; self._viewer_dirty = False; self.viewerChanged.emit(); return False
        base = Path(self._project).resolve()
        try: record["relative"] = str(Path(record["path"]).relative_to(base))
        except ValueError: record["relative"] = record["name"]
        self._viewer = record
        ...
        if record.get("image"):
            self._preview = {"kind": "image", "title": record["name"], "image": record["image"], "path": record["path"]}
            self.previewChanged.emit()
        return True

    @Slot(result=bool)
    def closeFile(self):
        if self._block_dirty_transition("closing the file"): return False
        self._viewer = {}; self._viewer_buffer = ""; self._viewer_dirty = False
        self.tree.set_selected(""); self.viewerChanged.emit(); return True

    @Slot(str)
    def setFileBuffer(self, text):
        text = str(text)
        if text == self._viewer_buffer: return
        self._viewer_buffer = text
        dirty = text != self._viewer.get("text", "")
        if dirty != self._viewer_dirty:
            self._viewer_dirty = dirty; self.viewerChanged.emit()

    @Slot(result=bool)
    def saveFile(self):
        if not self._viewer_dirty or not self._viewer.get("path"): return False
        try: files.write_file(self._project, self._viewer["path"], self._viewer_buffer)
        except (OSError, ValueError) as exc:
            self.toast.emit(files.explain(exc, f"“{self._viewer.get('name', 'That file')}”")); return False
        self._viewer["text"] = self._viewer_buffer
        self._viewer_dirty = False; self.viewerChanged.emit()
        self.toast.emit(f"Saved {self._viewer.get('name', 'file')}")
        self.refreshChanges(); return True

    @Slot()
    def revertFileBuffer(self):
        self._viewer_buffer = self._viewer.get("text", "")
        if self._viewer_dirty: self._viewer_dirty = False
        self.viewerChanged.emit()

    @Property(QObject, constant=True)
    def terminalModel(self): return self.lines

    @Property(bool, notify=terminalChanged)
    def terminalRunning(self): return bool(self._shell and self._shell.running)

    @Property(str, notify=terminalChanged)
    def terminalShell(self): return self._shell.shell_name if self._shell else ""

    @Property(str, notify=terminalChanged)
    def terminalDirectory(self): return self._shell_cwd
    ...
    def terminalDirectoryLabel(self):
        ...
        if not path: return ""
        try:
            home = str(Path.home()); return "~" + path[len(home):] if path.startswith(home) else path
        except (OSError, RuntimeError): return path

    @Property(str, notify=terminalChanged)
    def terminalError(self): return self._shell_error

    @Property(bool, notify=terminalChanged)
    def terminalStarted(self): return self._terminal_started

    @Slot("QVariantMap")
    def setTerminalPalette(self, palette):
        self.lines.set_palette(dict(palette or {}))
        if self._shell is not None: self.lines.sync(self._shell.screen.rows())

    @Slot()
    def startTerminal(self):
        if self._shell is not None and self._shell.running: return
        self._teardown_shell(); session = ShellSession(cwd=self._project or str(Path.home()))
        try: session.start()
        except Exception as exc:
            self._shell_error = f"Could not start a shell: {exc}"; self._terminal_started = True; self.terminalChanged.emit(); return
        self._shell = session; self._shell_error = ""; self._shell_cwd = session.cwd; self._terminal_started = True
        self._notifier = QSocketNotifier(session.fd, QSocketNotifier.Read, self)
        self._notifier.activated.connect(self._drain_shell); self.terminalChanged.emit()

    @Slot()
    def restartTerminal(self):
        self._teardown_shell(); self.lines.clear(); self._terminal_started = False; self.terminalChanged.emit()
        if self._visible and self._tab == "terminal": self.startTerminal()

    def _teardown_shell(self) -> None:
        if self._notifier is not None:
            self._notifier.setEnabled(False); self._notifier.deleteLater(); self._notifier = None
        if self._shell is not None:
            self._shell.stop(); self._shell = None

    def _drain_shell(self, *_):
        if self._shell is None: return
        produced = self._shell.read()
        if produced: self.lines.sync(self._shell.screen.rows())
        if not self._shell.running:
            if self._notifier is not None: self._notifier.setEnabled(False)
            self.terminalChanged.emit()

    def _poll_shell(self):
        if self._shell is None or not self._shell.running: return
        directory = self._shell.working_directory()
        if directory != self._shell_cwd:
            self._shell_cwd = directory; self.terminalChanged.emit()

    @Slot(str)
    def sendTerminal(self, text):
        if self._shell is None or not self._shell.running: self.startTerminal()
        if self._shell is None: return
        self._shell.send_line(str(text))

    @Slot(str)
    def writeTerminal(self, text):
        if self._shell is not None: self._shell.write(str(text))
    ...
    def runInTerminal(self, command):
        ...
        if self._tab != "terminal" or not self._visible:
            self._tab_pinned = True; self._remember("dock_tab_pinned", True); self._select_tab("terminal"); self.setVisible(True); self.changed.emit()
        self.startTerminal(); self.sendTerminal(command)

    @Slot()
    def interruptTerminal(self):
        if self._shell is not None: self._shell.interrupt()

    @Slot()
    def clearTerminal(self):
        if self._shell is not None: self._shell.screen.clear()
        self.lines.clear()

    @Slot(result=str)
    def terminalText(self): return self._shell.screen.plain_text() if self._shell else ""

    @Slot(int, int)
    def resizeTerminal(self, columns, rows):
        if self._shell is not None: self._shell.resize(columns, rows)

    @Property("QVariantList", notify=changesChanged)
    def changes(self): return self._changes.get("files", [])

    @Property(bool, notify=changesChanged)
    def isRepository(self): return bool(self._changes.get("repository"))

    @Property(str, notify=changesChanged)
    def branch(self): return str(self._changes.get("branch", ""))

    @Property(str, notify=changesChanged)
    def changesSummary(self):
        entries = self._changes.get("files", [])
        if not entries: return "No uncommitted changes"
        count = f"{len(entries)} file" + ("" if len(entries) == 1 else "s")
        return f"{count} · +{self._changes.get('added', 0)} −{self._changes.get('removed', 0)}"

    @Property(str, notify=changesChanged)
    def changesError(self): return str(self._changes.get("error", ""))

    @Property(bool, notify=changesChanged)
    def changesBusy(self): return self._changes_busy

    @Property(str, notify=changesChanged)
    def diffPath(self): return self._change_path

    @Property("QVariantList", notify=changesChanged)
    def diffRows(self): return self._diff.get("rows", [])

    @Property(str, notify=changesChanged)
    def diffError(self): return str(self._diff.get("error", ""))

    def _run(self, fn, done):
        worker = _Worker(fn, self); worker.callback = done
        worker.done.connect(self._deliver); worker.failed.connect(self._deliver_failure); worker.finished.connect(self._retire_worker)
        self._workers.add(worker); worker.start(); return worker
    ...
    @Slot(str)
    def _deliver_failure(self, message): self.toast.emit(message)

    @Slot()
    def _retire_worker(self):
        worker = self.sender(); self._workers.discard(worker)
        if worker is not None: worker.deleteLater()

    @Slot()
    def refreshChanges(self):
        if not self._project or self._changes_busy: return
        self._changes_busy = True; self.changesChanged.emit(); project = self._project
        def finish(result):
            self._changes_busy = False
            if project == self._project:
                self._changes = result; base = Path(project).resolve()
                self.tree.set_dirty({str(base / entry["path"]) for entry in result["files"]})
                if self._change_path and not any(entry["path"] == self._change_path for entry in result["files"]):
                    self._change_path = ""; self._diff = {"rows": [], "error": "", "path": "", "binary": False}
    ...
    def openDiff(self, path):
        path = str(path or "")
        if not self._project or not path: return
        self._change_path = path
        untracked = any(entry["path"] == path and entry.get("untracked") for entry in self._changes.get("files", []))
        self._diff = {"rows": [], "error": "Loading…", "path": path, "binary": False}; self.changesChanged.emit(); project = self._project
        def finish(result):
            if project == self._project and self._change_path == path: self._diff = result
            self.changesChanged.emit()
        self._run(lambda: diffs.file_diff(project, path, untracked), finish)

    @Slot()
    def closeDiff(self): self._change_path = ""; self._diff = {"rows": [], "error": "", "path": "", "binary": False}; self.changesChanged.emit()
    ...
    def revertChange(self, path):
        ...
        if not result.get("ok"):
            self.toast.emit(result.get("error") or "Could not revert that file"); return False
        ...
        if self._viewer.get("path"):
            try:
                if Path(self._viewer["path"]) == Path(self._project) / path:
                    self.openFile(self._viewer["path"]) if Path(self._viewer["path"]).exists() else self.closeFile()
            except (OSError, ValueError): pass
        self.closeDiff(); self.refreshChanges(); return True

    @Slot(str, result=str)
    def absolutePath(self, relative):
        if not self._project: return ""
        try: return str(files.resolve_within(self._project, str(relative)))
        except (OSError, ValueError): return ""

    @Property("QVariantList", notify=contextChanged)
    def contextItems(self):
        groups: list[dict] = []; owner = self._owner
        if self._project:
            groups.append({"title": "Workspace", "icon": "folderOpen", "items": [{"id": "project", "label": Path(self._project).name, "detail": self._project, "removable": False, "kind": "folder"}]})
        attachments = list(getattr(owner, "_attachments", []) or []) if owner else []
        if attachments:
            groups.append({"title": "Attached", "icon": "paperclip", "items": [{"id": "attachment:" + str(item.get("id", "")), "label": str(item.get("title", "")), "detail": str(item.get("subtitle", "")), "removable": True, "kind": str(item.get("kind", "file")), "tokens": int(item.get("tokens", 0) or 0)} for item in attachments]})
        if self._viewer.get("path"):
            groups.append({"title": "Open file", "icon": "file", "items": [{"id": "file", "label": self._viewer.get("name", ""), "detail": self._viewer.get("relative", ""), "removable": True, "kind": "file", "tokens": 0}]})
        if self._browser_url:
            groups.append({"title": "Browser", "icon": "globe", "items": [{"id": "browser", "label": self._browser_title or browser_policy.display_url(self._browser_url), "detail": browser_policy.display_url(self._browser_url), "removable": True, "kind": "link"}]})
        changed = self._changes.get("files", [])
        if changed:
            groups.append({"title": "Changed files", "icon": "branch", "items": [{"id": "change:" + entry["path"], "label": entry["name"], "detail": f"+{entry['added']} −{entry['removed']}", "removable": False, "kind": "diff"} for entry in changed[:12]]})
        return groups

    @Slot(str)
    def removeContext(self, identifier):
        identifier = str(identifier or "")
        if identifier == "file": self.closeFile()
        elif identifier == "browser": self.clearBrowser()
        elif identifier.startswith("attachment:") and self._owner is not None:
            remove = getattr(self._owner, "removeAttachment", None)
            if callable(remove): remove(identifier.split(":", 1)[1])
        self.contextChanged.emit()

    def refresh_context(self) -> None: self.contextChanged.emit()

    @Property("QVariantList", notify=activityChanged)
    def activityRows(self): return self.log.rows()

    @Property(str, notify=activityChanged)
    def activitySummary(self): return self.log.summary()

    @Property(bool, notify=activityChanged)
    def activityRunning(self): return self.log.running

    @Slot()
    def clearActivity(self): self.log.clear(); self.activityChanged.emit()

    def record(self, event: dict) -> None: self.log.append(event); self.activityChanged.emit()
    def record_update(self, **fields) -> None: self.log.update_last(**fields); self.activityChanged.emit()
    def begin_turn(self, title: str) -> None: self.log.begin_turn(title); self.activityChanged.emit()
    def settle_turn(self, state: str) -> None: self.log.settle_turn(state); self.activityChanged.emit()

    @Property(bool, constant=True)
    def browserAvailable(self): return browser_policy.engine_available()

    @Property(str, constant=True)
    def browserUnavailableReason(self): return browser_policy.unavailable_reason()

    @Property(str, constant=True)
    def browserHome(self): return browser_policy.DEFAULT_HOME

    @Property(str, notify=browserChanged)
    def browserUrl(self): return self._browser_url

    @Property(str, notify=browserChanged)
    def browserRequest(self): return self._browser_pending

    @Property(str, notify=browserChanged)
    def browserDisplayUrl(self): return browser_policy.display_url(self._browser_url)

    @Property(str, notify=browserChanged)
    def browserTitle(self): return self._browser_title

    @Property(bool, notify=browserChanged)
    def browserLoading(self): return self._browser_loading

    @Property(float, notify=browserChanged)
    def browserProgress(self): return self._browser_progress

    @Property(bool, notify=browserChanged)
    def browserSecure(self): return browser_policy.is_secure(self._browser_url)

    @Property(str, notify=browserChanged)
    def browserTrust(self):
        if not self._browser_url: return ""
        if browser_policy.is_secure(self._browser_url): return "secure"
        return "local" if browser_policy.is_local(self._browser_url) else "insecure"

    @Property(str, notify=browserChanged)
    def browserError(self): return self._browser_error

    @Slot(str, result=bool)
    def navigate(self, text):
        target = browser_policy.normalize(text)
        if not target:
            self._browser_error = "That is not an address Wynxo can open"; self.browserChanged.emit(); return False
        self._browser_error = ""; self._browser_pending = target; self._browser_loading = True; self.browserChanged.emit(); return True
    ...
    def browserStateChanged(self, url, title):
        ...
        self.browserChanged.emit(); self.contextChanged.emit()

    @Slot(bool, float)
    def browserLoadState(self, loading, progress):
        self._browser_loading = bool(loading); self._browser_progress = max(0.0, min(float(progress or 0), 1.0)); self.browserChanged.emit()

    @Slot(bool, bool)
    def browserHistoryState(self, back, forward):
        self._browser_can_back = bool(back); self._browser_can_forward = bool(forward); self.browserChanged.emit()

    @Property(bool, notify=browserChanged)
    def browserCanGoBack(self): return self._browser_can_back

    @Property(bool, notify=browserChanged)
    def browserCanGoForward(self): return self._browser_can_forward

    @Slot(str)
    def browserFailed(self, message): self._browser_error = str(message or ""); self._browser_loading = False; self.browserChanged.emit()

    @Slot()
    def clearBrowser(self):
        self._browser_url = ""; self._browser_title = ""; self._browser_pending = ""; self._browser_error = ""; self.browserChanged.emit(); self.contextChanged.emit()

    @Slot(str, str, str)
    def attachPage(self, url, title, text):
        owner = self._owner
        if owner is None: return
        page = browser_policy.page_context(url, title, text); attach = getattr(owner, "attach_web_page", None)
        if callable(attach): attach(page); self.contextChanged.emit()

    @Property("QVariantMap", notify=previewChanged)
    def preview(self): return dict(self._preview)

    @Slot(str, str, str)
    def showPreview(self, kind, title, payload):
        kind = str(kind or "")
        if kind not in ("image", "markdown", "html", "text"): return
        self._preview = {"kind": kind, "title": str(title or ""), "body": str(payload)}
        if kind == "image": self._preview["image"] = str(payload)
        self.previewChanged.emit(); self.suggest("preview", open_dock=True)

    @Slot()
    def clearPreview(self): self._preview = {}; self.previewChanged.emit()

    def shutdown(self) -> None:
        self._search_generation += 1
        self._search_debounce.stop()
        self._poll.stop(); self._teardown_shell()
        for worker in list(self._workers):
            worker.quit(); finished = worker.wait(2000)
            for signal in (worker.done, worker.failed, worker.finished):
                try: signal.disconnect()
                except (RuntimeError, TypeError): pass
            if finished:
                worker.setParent(None); continue
            worker.setParent(None); _ORPHANED.add(worker); worker.finished.connect(lambda w=worker: _ORPHANED.discard(w))
        self._workers.clear()
